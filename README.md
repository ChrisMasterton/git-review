...
- One-click cleanup for local branches whose upstream is gone. Cleanup uses Git's safe delete and skips checked-out or unmerged branches rather than force-deleting work.
...
- Inline branch actions: safely clean up an individual branch whose upstream is gone, or push any branch that is ahead without checking it out first.
...
Push publishes existing commits from the current branch without staging or committing working-tree changes. It uses the existing branch remote when configured and creates an `origin` upstream for a new local branch. Pull is available only for clean working trees and runs with `--ff-only`, so it never creates an implicit merge commit.
